...
def get_active_users(browser, username, posts, boundary, logger):
    ...
    try:
        total_posts = browser.find_element(By.XPATH, "//span[@class='_ac2a']/span").text
...
def format_number(number):
    ...
    if 'K' in formatted_num:
        formatted_num = formatted_num.replace('K', '')
        formatted_num = float(formatted_num) * 1000
    else:
...
def get_relationship_counts(browser, username, logger):
    ...
    try:
        followers_count = browser.find_elements(By.XPATH, f"//a[contains(@href,'/{username}/followers/')]/span/span")
        if len(followers_count) == 1:
            followers_count = followers_count[0].text.replace('followers', '')
        else:
            followers_count = followers_count[1].text.replace('followers', '')
        ...
        try:
            ...
            followers_count = getUserData("graphql.user.edge_followed_by.count", browser)
    ...
    try:
        following_count = browser.find_elements(By.XPATH, f"//a[contains(@href,'/{username}/following/')]/span/span")
        ...
        try:   
            following_count = getUserData("graphql.user.edge_follow.count", browser)         
...
def save_account_progress(browser, username, logger, followed_by, following_num):
    """
    ...
    followers, following = followed_by, following_num #get_relationship_counts(browser, username, logger)
