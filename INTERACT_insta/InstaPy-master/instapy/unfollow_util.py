...
def unfollow(
    ...
    # my simplified version of this unfollow function (danidan)
    unfollowed = []
    if followings != []:
        some_100followers = [list(followers)[i] for i in random.sample(range(0, len(followers)), 100)]
        followings = [p for p in followings if p not in some_100followers] 
        for person in followings:
            ...
            if time_diff > delay_followbackers:
                ...
                try:
                    ...
                    unfollowed.append(person)
                    unfollow_num = unfollow_num + 1
            ...
            if unfollow_num > amount:
                return unfollow_num , unfollowed                 
        return unfollow_num , unfollowed
...
def get_users_through_dialog_with_graphql(
    ...
    # QUICK FOLLOW : directly follow from the dialog box
    if quick_follow == True:
        time.sleep(2)
        do_again = True
        limit = amount * 2
        st = time.time()
        while do_again == True and time.time()-st<10:
            list_all_followers = browser.find_elements(By.XPATH, "//a/div/div/span")
            list_all_followers = list_all_followers[:limit]
            persons_to_follow = [p.text for p in list_all_followers]
            persons_to_follow = [p for p in persons_to_follow if p not in my_person_list]
            if len(persons_to_follow) < amount:
                do_again = True
                limit = limit * 2
            else:
                do_again = False
        if persons_to_follow == []:
            logger.info("INSTAGRAM DOES NOT LET US KNOW THE FOLLOWER LIST FOR NOW :((")
            return([], [])
        persons_to_follow = persons_to_follow[:amount]
        persons_followed = []
        for person in persons_to_follow:
            try:
                scroll_to_bottom_of_followers_list(browser)
                print(person)
                div_button = browser.find_element(By.XPATH, f"//div[a/div/div/span[text()='{person}']]")
                button = div_button.find_element(By.XPATH, "//button[div/div[text()= 'Follow']]")   #"//button/div/div[text()= 'Follow']"
                button.click()
                time.sleep(random.random()+0.5)
                persons_followed.append(person)
            except:
                try:
        ...
        button_to_follow = browser.find_elements(By.XPATH, "//button/div/div[text()= 'Follow']")
        for button in button_to_follow[:amount]:
            button.click()
    ...
    while has_next_page and len(followers_list) <= amount:
        ...
        #     list_of_new_followers = [p for p in followers_list if p not in my_person_list]            
        #     quick_amount = 1 if not len(list_of_new_followers) >= amount else random.randint(1, 4)
    ...
    real_amount = min(len(followers_list), real_amount)       
    # get real amount
...
def get_given_user_followers(
    ...
    if not allfollowers:
        ...
        amount = min(amount, allfollowers)
...
def get_given_user_following(
    ...
    try:
        ...
        try:
            _, allfollowing = get_relationship_counts(browser, user_name, logger)
...
def confirm_unfollow(browser):
    ...
    while attempt < 3:
        try:
            ...
            # text(), 'Unfollow')]"
            unfollow_button = browser.find_element(By.XPATH,"//span[text()='Unfollow']")
            # browser.find_element(By.XPATH, button_xp)
