...
def get_links_for_tag(browser, tag, amount, skip_top_posts, randomize, media, logger):
    ...
    try:
        ...
        try: 
            ...
            # ).text
            xp_possible_posts = "//span[@class='html-span xdj266r x11i5rnm xat24cr x1mh8g0r xexx8yu x4uap5 x18d9i69 xkhd6sd x1hl2dhg x16tdsg8 x1vvkbs']"
            possible_posts = browser.find_element(
                By.XPATH, xp_possible_posts
            ).text
...
def like_image(browser, username, blacklist, logger, logfolder, total_liked_img):
    ...
    like_xpath = "//span[@class='xp7jhwk']/div[@role='button']" #read_xpath(like_image.__name__, "like")
    unlike_xpath = like_xpath # like_xpath # read_xpath(like_image.__name__, "unlike")
    ...
    try:
        ...
        try:
            photo_url = link + '?igshid=MzRIODBiNWFIZA=='
            web_address_navigator(self.browser, photo_url)
            like_elem = browser.find_elements(By.XPATH, like_xpath)
        except:
            photo_url = link + '?igshid=NzgyYTk0Y2YyNg==' 
            web_address_navigator(self.browser, photo_url)
            like_elem = browser.find_elements(By.XPATH, like_xpath)
...
def verify_liked_image(browser, logger):
    ...
    browser.refresh()
    unlike_xpath ="//span[@class='xp7jhwk']/div[@role='button']" # read_xpath(like_image.__name__, "unlike")
            ...
            for link_elem in link_elems:
                try:
                    ...
                    if len(post_elem) == 2:
                        ...
                        #     "//a[@href='/p/"
                        #     + post_href.split("/")[-2]
                        #     + "/']/div[contains(@class,'x9f619 xjbqb8w')]/div[contains(@class,'_aatp')]/div[contains(@class,'x9f619 xjbqb8w')]/*[name()='svg']",
                        # ).get_attribute("aria-label")
