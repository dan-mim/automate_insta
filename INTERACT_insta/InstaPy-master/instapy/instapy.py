...
class InstaPy:
    ...
    def __init__(
        ...
        split_db: bool = False,
        bypass_security_challenge_using: str = "sms",
    ...
    def get_instapy_logger(self, show_logs: bool, log_handler=None):
        ...
        if existing_logger is not None:
            ...
            for handler in logger.logger.handlers[:-1]:
    ...
    def like_by_tags(
        ...
        for index, tag in enumerate(tags):
            ...
            for i, link in enumerate(links):
                ...
                try:
                    # inappropriate, user_name, is_video, reason, scope = False, user_name, is_video, "None", "Success"
                    inappropriate, user_name, is_video, reason, scope = check_link(
                        self.browser,
                        link,
                        self.dont_like,
                    ...
                    if not inappropriate and self.liking_approved and (user_name not in self.my_person_list):
                        # validate user
                        # validation, details = self.validate_user_call(user_name)
                        # if validation is not True:
                        #     self.logger.info(details)
        ...
        for index, username in enumerate(usernames):
            ...
            if self.like_percentage > 0:
                try:
                    ...
                    self.logger.error("Element not found, I follow and I skip this username")
                    continue
            else:
                links = False
    ...
    def interact_user_followers(
        self, usernames: list, amount: int = 10, randomize: bool = False, quick_follow=True,
    ):
        """
    ...
    def interact_user_likers(
        ...
        for index, username in enumerate(usernames):
            ...
            for post_index, post_url in enumerate(post_urls):
                ...
                likers = users_liked(
                    self.browser, post_url, interact_likers_per_post, self.logger, self.my_person_list
                )
                # This way of iterating will prevent sleep interference
                # between functions
                random.shuffle(likers)
                ...
                likers = likers[:interact_likers_per_post]
    ...
    def unfollow_users(
        ...
        try:
            unfollowed_nb , l_unfollowed = unfollow(
                self.browser,
                self.username,
                amount,
            ...
            self.logger.info("--> Total people unfollowed : {}\n".format(unfollowed_nb))
            self.unfollowed += unfollowed_nb
            # followings list is updated:
            self.my_followings = [p for p in self.my_followings if p not in l_unfollowed]
